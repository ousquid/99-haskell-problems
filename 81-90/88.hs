...
connectedComponents :: Ord a => GraphTerm a -> [[a]]
connectedComponents graph = connectedComponents' graph []
    where connectedComponents' (Graph nodes edges) currentComponents = if rest == [] then newComponents else connectedComponents' (Graph rest edges) newComponents
            where component = depthFirst graph $ head nodes
                  rest = toList $ difference (fromList nodes) (fromList component)
